...
A=ggplot(soil, aes(x=longitude, y=latitude)) +
  ...
  xlab("") +
  ylab("") +
  guides(fill= guide_legend(title=expression(bold(paste('log'[10]," 2HPAA (pg/mg)")))), 
         size=guide_legend(title=expression(bold(paste('log'[10]," 2HPAA (pg/mg)"))))) +  
  theme(panel.grid.major = element_blank(),
        ...
        axis.title.y = element_text(margin = margin(t = 20,r = 20,b = 10, l = 10)),
...
annotate_figure(HPAA.bar,
                left = text_grob(expression(paste(bold('log'[10]),bold(" 2HPAA (pg/mg)"))), 
